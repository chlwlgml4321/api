...
public class UseEachImpl implements UseEach {

    private final String USE = " MPS.PKG_MPS_CUST_WLLT_V2.USE_EACH";
    @PersistenceContext
    private EntityManager entityManager;

    @Override
    public UseEachOut useEach(UseEachIn params) {
        ...
        //Out 변수
        Arrays.stream(UseEachOut.class.getDeclaredFields())
                .forEach(field -> {
                    field.setAccessible(true);
        ...
        query.setParameter("inMnyAmt", (Number) params.getInMnyAmt().longValue());
        query.setParameter("inPntAmt", (Number) params.getInPntAmt().longValue());
